...
namespace lidar_localization
{
    ...
    bool FrontEndFlow::InitSubscribers(ros::NodeHandle &nh, const YAML::Node &config_node)
    {
        ...
        // world_to_map_tf_pub_ptr_ = std::make_shared<TFBroadCaster>("/world", "/map");
    ...
    bool FrontEndFlow::UpdateLaserOdometry()
    {
        ...
        if (!front_end_pose_inited)
        {
            ...
            front_end_ptr_->SetInitPose(gnss_odometry_); // 初始帧是GPS数据
        }

        laser_odometry_ = Eigen::Matrix4f::Identity();
        // === using no GPS to licalization and map ===
        return front_end_ptr_->Update(current_cloud_data_, laser_odometry_);
        // === using full GPS pose to licalization and map ===
        // return front_end_ptr_->UpdatewithGPS(current_cloud_data_, laser_odometry_, gnss_odometry_);
        // === using part GPS pose(only xyz) to licalization and map ===
        // return front_end_ptr_->UpdatewithGPSxy(current_cloud_data_, laser_odometry_, gnss_odometry_);
    }

    bool FrontEndFlow::PublishData()
    {
        lidar_to_map_tf_pub_ptr_->SendTransform(laser_odometry_, current_cloud_data_.time);
        // world_to_map_tf_pub_ptr_->SendTransform(Eigen::MatrixXf::Identity(4, 4), current_cloud_data_.time);
