...
namespace lidar_localization
{
    ...
    bool FrontEnd::InitDataPath(const YAML::Node &config_node)
    {
        ...
        {
            LOG(INFO) << "Key Frames Output Path: " << key_frame_path << std::endl
                      << std::endl;
        }
    ...
    bool FrontEnd::UpdatewithGPSxy(const CloudData &cloud_data, Eigen::Matrix4f &cloud_pose, Eigen::Matrix4f &gps_pose)
    {
        ...
        // 局部地图容器中没有关键帧，代表是第一帧数据（第一帧是关键帧，添加到局部地图中）
        ...
        // 不是第一帧，就正常匹配，当前帧-匹配-局部地图（local map）
        // result_cloud_ptr_为filtered_cloud_ptr变换到local map中的点云，由于filtered，所以在RViz中呈现栅格点的样子
        registration_ptr_->ScanMatch(filtered_cloud_ptr, predict_pose, result_cloud_ptr_, current_frame_.pose);
        // ADD: make the estimiated pose's xyz to be the gps data's xyz
        current_frame_.pose(0, 3) = gps_pose(0, 3);
        current_frame_.pose(1, 3) = gps_pose(1, 3);
        current_frame_.pose(2, 3) = gps_pose(2, 3);
        ...
        // 匹配之后根据距离判断是否需要生成新的关键帧，如果需要，则做相应更新（建图）
    ...
    bool FrontEnd::UpdatewithGPS(const CloudData &cloud_data, Eigen::Matrix4f &cloud_pose, Eigen::Matrix4f &gps_pose)
    {
        current_frame_.cloud_data.time = cloud_data.time;
        std::vector<int> indices;
        pcl::removeNaNFromPointCloud(*cloud_data.cloud_ptr, *current_frame_.cloud_data.cloud_ptr, indices);
        ...
        pcl::transformPointCloud(*filtered_cloud_ptr, *result_cloud_ptr_, gps_pose);
    ...
    bool FrontEnd::Update(const CloudData &cloud_data, Eigen::Matrix4f &cloud_pose)
    {
        ...
        // 不是第一帧，就正常匹配，当前帧-匹配-局部地图（local map）
        // result_cloud_ptr_为filtered_cloud_ptr变换到local map中的点云，由于filtered并且对齐，所以在RViz中呈现栅格点的样子
        registration_ptr_->ScanMatch(filtered_cloud_ptr, predict_pose, result_cloud_ptr_, current_frame_.pose);
        cloud_pose = current_frame_.pose;
    ...
    bool FrontEnd::UpdateWithNewFrame(const Frame &new_key_frame)
    {
        ...
        // 更新局部地图，这部分设计挺创新的！
        local_map_frames_.push_back(key_frame); //更新维护的是frame pointcloud.
