...
static VolumetricModelParams getVolumetricModelParams(
    ...
    nh.param<bool>(volumetric_ns + "/use_water_obstacle_volumetric_model",
                   params.use_water_obstacle_model, false);
    std::vector<double> volumetric_model(6, 0.);
    nh.getParam(volumetric_ns + "/water_obstacle_volumetric_model",
                volumetric_model);
    params.model_water_obstacle.model_type = WATER_OBSTACLE;
    params.model_water_obstacle.l_min = volumetric_model[0];
    params.model_water_obstacle.l_max = volumetric_model[1];
    params.model_water_obstacle.w_min = volumetric_model[2];
    params.model_water_obstacle.w_max = volumetric_model[3];
    params.model_water_obstacle.h_min = volumetric_model[4];
    params.model_water_obstacle.h_max = volumetric_model[5];
    displayModelInfo(params.model_water_obstacle);

    return params;
}

static NoiseParams getNoiseParams(const ros::NodeHandle& nh,
                                  const std::string& ns_prefix) {
    NoiseParams params;

    const std::string ns = ns_prefix + "/noise";

    nh.getParam(ns + "/search_radius_m", params.search_radius_m);
    nh.getParam(ns + "/min_point_number_in_radius_m",
...
static std::vector<autosense::MapROIParams> getMapROIParams(
    const ros::NodeHandle& nh, const std::string& ns_prefix) {
    std::vector<autosense::MapROIParams> params;
    const std::string ns = ns_prefix + "/MapRoi";
    double rows, cols;
    nh.getParam(ns + "/rows", rows);
    nh.getParam(ns + "/cols", cols);
    std::vector<double> data;
    nh.getParam(ns + "/data", data);
    int vertexes_num = rows;
    for (int vtx_i = 0; vtx_i < vertexes_num; vtx_i++) {
        autosense::MapROIParams vertex;
        vertex.x = data[vtx_i * 2];
        vertex.y = data[vtx_i * 2 + 1];
        params.push_back(vertex);
    }
    return params;
}
