...
/**
 * @brief publish Objects Vision Message
 * @param publisher
 * @param header
 * @param objects
 */
static void publishObjectsMsg(const ros::Publisher &publisher,
                              const std_msgs::Header &header,
                              const std::vector<ObjectPtr> &objects_array) {
    vision_msgs::Detection3DArray objects_ros_msg;
    objects_ros_msg.header = header;
    for (size_t obj = 0u; obj < objects_array.size(); ++obj) {
        vision_msgs::Detection3D object_ros;
        ...
        object_ros.header = header;
        object_ros.bbox.center.position.x = center[0];
        object_ros.bbox.center.position.y = center[1];
        object_ros.bbox.center.position.z = center[2];
        object_ros.bbox.center.orientation =
            tf::createQuaternionMsgFromRollPitchYaw(0, 0, yaw);
        object_ros.bbox.size.x = length;
        object_ros.bbox.size.y = width;
        object_ros.bbox.size.z = height;

        pcl::toROSMsg(*objects_array[obj]->cloud, object_ros.source_cloud);

        objects_ros_msg.detections.push_back(object_ros);
    }
    publisher.publish(objects_ros_msg);
}
...
static void publishObjectsVelocityArrow(
    ...
    if (!objects_array.empty()) {
        ...
        for (size_t obj = 0u; obj < objects_array.size(); ++obj) {
            ...
            const Eigen::Vector3d &real_velocity = objects_array[obj]->velocity;
            Eigen::Vector3d velocity = real_velocity * viz_scale;  // yujie.jin
            ...
            vel.scale.x = 0.6;
            vel.scale.y = 1.2;
            /// 30% velocity scalar as arrow header
            double vel_scalar =
                sqrt(pow(velocity[0], 2.0) + pow(velocity[1], 2.0));
            vel.scale.z = vel_scalar * 0.3;
            vel.color = color;
...
static void publishRangeMapROI(
    const ros::Publisher &publisher,
    const std_msgs::Header &header,
    const std::vector<autosense::MapROIParams> map_roi_vertexes) {
    if (map_roi_vertexes.empty()) {
        ROS_WARN("Publish empty fixed trajectory.");
        return;
    } else {
        ROS_INFO_STREAM("Publishing " << map_roi_vertexes.size()
                                      << " fixed roi range points.");
    }

    if (!map_roi_vertexes.empty()) {
        std::vector<geometry_msgs::Point32> polygon_geo_vertexes;
        for (int vtx_i = 0; vtx_i < map_roi_vertexes.size(); vtx_i++) {
            geometry_msgs::Point32 geo_pt;
            geo_pt.x = map_roi_vertexes[vtx_i].x;
            geo_pt.y = map_roi_vertexes[vtx_i].y;
            geo_pt.z = 0;
            polygon_geo_vertexes.push_back(geo_pt);
        }

        geometry_msgs::PolygonStamped polygonMsg;
        polygonMsg.header = header;
        polygonMsg.header.frame_id = "world";
