...
namespace object_map
{
	...
	{
		InitializeROSIo();
		LoadWharfAreasFromVectorMap(private_node_handle_, unpassed_area_points_);
		LoadWaterAreasFromVectorMap(private_node_handle_, water_area_points_);
	}

	void WayareaToGrid::InitializeROSIo()
	{
		private_node_handle_.param<std::string>("tf_sensor_frame", tf_sensor_frame_, "velo_link");
