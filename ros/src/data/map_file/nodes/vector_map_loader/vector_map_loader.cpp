...
using vector_map::WaterAreaArray;
...
int main(int argc, char **argv)
{
  ...
  // 3th param: latch	(optional) If true, the last message published on this topic will be saved and sent to new subscribers when they connect
  ...
  ros::Publisher water_area_pub = nh.advertise<WaterAreaArray>("vector_map_info/water_area", 1, true);
  ...
  for (const auto &file_path : file_paths)
  {
    ...
    else if (file_name == "waterarea.csv")
    {
      water_area_pub.publish(createObjectArray<WaterArea, WaterAreaArray>(file_path));
      category |= Category::WATER_AREA;
    }
    else
  ...
  insertMarkerArray(marker_array, createRoadEdgeMarkerArray(vmap, Color::LIGHT_CYAN));
  ...
  insertMarkerArray(marker_array, createStopLineMarkerArray(vmap, Color::LIGHT_RED));
  insertMarkerArray(marker_array, createZebraZoneMarkerArray(vmap, Color::WHITE));
  insertMarkerArray(marker_array, createCrossWalkMarkerArray(vmap, Color::LIGHT_YELLOW));
