...
namespace vector_map
{
  namespace
  {
    ...
    void updateWaterArea(std::map<Key<WaterArea>, WaterArea> &map, const WaterAreaArray &msg)
    {
      map = std::map<Key<WaterArea>, WaterArea>();
      for (const auto &item : msg.data)
      {
        if (item.id == 0)
          continue;
        map.insert(std::make_pair(Key<WaterArea>(item.id), item));
      }
    }
  } // namespace
  ...
  void VectorMap::registerSubscriber(ros::NodeHandle &nh, category_t category)
  {
    ...
    if (category & WATER_AREA)
    {
      water_area_.registerSubscriber(nh, "/vector_map_info/water_area");
      water_area_.registerUpdater(updateWaterArea);
    }
  }
  ...
  WaterArea VectorMap::findByKey(const Key<WaterArea> &key) const
  {
    return water_area_.findByKey(key);
  }
  ...
  std::vector<WaterArea> VectorMap::findByFilter(const Filter<WaterArea> &filter) const
  {
    return water_area_.findByFilter(filter);
  }
  ...
  void VectorMap::registerCallback(const Callback<WaterAreaArray> &cb)
  {
    water_area_.registerCallback(cb);
  }
  ...
  extern const double MAKER_SCALE_LINE = 0.5;
  extern const double MAKER_SCALE_AREA = 0.5;
...
std::ostream &operator<<(std::ostream &os, const vector_map::WaterArea &obj)
{
  os << obj.id << ","
     << obj.kid << ","
     << obj.slid << ","
...
std::istream &operator>>(std::istream &is, vector_map::WaterArea &obj)
{
  std::vector<std::string> columns;
  std::string column;
  while (std::getline(is, column, ','))
  ...
  obj.id = std::stoi(columns[0]);
  obj.kid = std::stoi(columns[1]);
  obj.slid = std::stoi(columns[2]);
  obj.elid = std::stoi(columns[3]);
  obj.connectid = std::stoi(columns[4]);
  return is;
}
